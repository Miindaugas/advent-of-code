...
const area = [...Array(columns)].map(_ => Array(rows).fill(0));
...
for (let i = 0; i < input.length; i = i + 2) {
    ...
    const [x2, y2] = input[i + 1].split(",")
    pipes.push({x: Number(x), y: Number(y), x2: Number(x2), y2: Number(y2)});
}
...
const fillArea = (pipe) => {
    ...
    if (pipe.x === pipe.x2) {
        direction = pipe.y < pipe.y2 ? Directions.down : Directions.up;
        size = Math.abs(pipe.y - pipe.y2);
    }

    if (pipe.y === pipe.y2) {
        direction = pipe.x < pipe.x2 ? Directions.right : Directions.left;
        size = Math.abs(pipe.x - pipe.x2);
    }

    if (pipe.x !== pipe.x2 && pipe.y !== pipe.y2) {
        direction =
            (pipe.y < pipe.y2 ? Directions.down : Directions.up) |
            (pipe.x < pipe.x2 ? Directions.right : Directions.left);
        size = (Math.abs(pipe.y - pipe.y2) + Math.abs(pipe.x - pipe.x2)) / 2;
    }
        ...
        for (let i = 0; i <= size; i++) {
            ...
            switch (direction) {
                case Directions.down:
                    area[pipe.x][pipe.y + i]++;
                    break;
                case Directions.up:
                    area[pipe.x][pipe.y - i]++;
                    break;
                case Directions.left:
                    area[pipe.x - i][pipe.y]++;
                    break;
            ...
            if (cross) {
                switch (true) {
                    case (direction & Directions.down && direction & Directions.left) > 0:
                        area[pipe.x - i][pipe.y + i]++;
                        break;
                    case (direction & Directions.down && direction & Directions.right) > 0:
                        area[pipe.x + i][pipe.y + i]++;
                        break;
                    case (direction & Directions.up && direction & Directions.left) > 0:
                        area[pipe.x - i][pipe.y - i]++;
                        break;
                    case (direction & Directions.up && direction & Directions.right) > 0:
                        area[pipe.x + i][pipe.y - i]++;
                        break;
                    default:
                        throw "Cross is not even";
                }
            }
        }
    }
...
for (let y = 0; y < rows; y++) {
    for (let x = 0; x < columns; x++) {
        if (area[x][y] >= 2) counter++;
        map += `${area[x][y]}`;
    }
    map += `\n`;
}

if (visual) console.log(map);
